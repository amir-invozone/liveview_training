defmodule LiveTrainingWeb.SurveyLive.RatingLive.Index do
  use Phoenix.Component
  use Phoenix.HTML

  alias LiveTrainingWeb.SurveyLive.RatingLive
  ...
  def list(assigns) do
    ...
    <%= for {game, index} <- Enum.with_index(@games) do %>
      <%= if rating = List.first(game.ratings) do %>
        <LiveTrainingWeb.SurveyLive.RatingLive.Show.stars rating={rating} game={game} />
      <% else %>
